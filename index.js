...
const {downloadmenu} = require('./X1/downloadmenu')
...
audio35 = fs.readFileSync('./X4/nyz35.m4a') 
			...
			mess = {
			wait: '𝘞𝘢𝘪𝘵 𝘒𝘢𝘬 𝘈𝘬𝘢𝘯 𝘚𝘢𝘺𝘢 𝘗𝘳𝘰𝘴𝘦𝘴',
			success: '𝘋𝘰𝘯𝘦 𝘒𝘢𝘬',
			error: {
	        bug: '𝘛𝘦𝘳𝘫𝘢𝘥𝘪 𝘒𝘦𝘴𝘢𝘭𝘢𝘩𝘢𝘯',
			stick: ' *𝘔𝘢𝘢𝘧 𝘒𝘢𝘬 𝘐𝘵𝘶 𝘉𝘶𝘬𝘢𝘯 𝘚𝘵𝘪𝘤𝘬𝘦𝘳 !!*',
			Iv: '*𝘓𝘪𝘯𝘬 𝘛𝘪𝘥𝘢𝘬 𝘝𝘢𝘭𝘪𝘥*'
			},
			only: {
			group: '*𝘍𝘪𝘵𝘶𝘳 𝘒𝘩𝘶𝘴𝘶𝘴 𝘎𝘳𝘶𝘱 !!*',
			benned: '*𝘔𝘢𝘢𝘧 𝘒𝘢𝘬 𝘒𝘢𝘮𝘶 𝘛𝘦𝘳𝘣𝘢𝘯𝘯𝘦𝘥 !!*',
			ownerG: '*𝘖𝘯𝘭𝘺 𝘖𝘸𝘯𝘦𝘳 𝘈𝘥𝘮𝘪𝘯 𝘎𝘳𝘶𝘱*',
			ownerB: '*𝘍𝘪𝘵𝘶𝘳 𝘒𝘩𝘶𝘴𝘶𝘴 𝘖𝘸𝘯𝘦𝘳 𝘒𝘢𝘬* ',
			premium: '*𝘍𝘪𝘵𝘶𝘳 𝘒𝘩𝘶𝘴𝘶𝘴 𝘔𝘦𝘮𝘣𝘦𝘳 𝘗𝘳𝘦𝘮𝘪𝘶𝘮 𝘒𝘢𝘬*',			 
			admin: '*𝘍𝘪𝘵𝘶𝘳 𝘒𝘩𝘶𝘴𝘶𝘴 𝘈𝘥𝘮𝘪𝘯 𝘎𝘳𝘶𝘱*',
			Badmin: '*𝘉𝘰𝘵 𝘉𝘶𝘬𝘢𝘯 𝘈𝘥𝘮𝘪𝘯 𝘒𝘢𝘬 !!*',
			}
			}			
            ...
            break				    		 
		    case 'downloadmenu':		    
		    helpzzzzz2 = fs.readFileSync(`./X3/help.jpeg`) 
            const helpzzzzz3 = await nayla.prepareMessage(from, helpzzzzz2, MessageType.image, {thumbnail:Buffer.alloc(0)}) 
            const helpzzzzz4 = [{buttonId: 'id1', buttonText: {displayText: 'DONASI'}, type: 1},{buttonId: 'id2', buttonText: {displayText: 'INFOBOT'}, type: 1},{buttonId: 'id3', buttonText: {displayText: 'OWNERBOT'}, type: 1}]                          
            const helpzzzzz5 = {            
            contentText: downloadmenu(tz, nama, namabot, prefix, day, yy, thisDay, bulan, wib, wita, wit),
            footerText: `${nama}\n ${tz} RUNTIME : ${kyun(runtime)}`,
            buttons: helpzzzzz4,
            headerType: 4,
            thumbnail:Buffer.alloc(0),
            imageMessage: helpzzzzz3.message.imageMessage
            }
            nayla.sendMessage(from, helpzzzzz5, MessageType.buttonsMessage, {quoted: nay1})
		    break				    		 
		    case 'imageanime':		    
            ...
            break
            case 'ytmp3': 
             if (args.length < 1) return query("https://youtu.be/cfymgLi2CZc")
            nay2 = await fetchJson(`${restv01}/api/ytmp3/2?apikey=${apiv01}&url=${aq}`)
            reply(mess.wait)
            nay3 = await getBuffer(nay2.result.link)
            nay5 = await getBuffer(nay2.result.thumb)
            nay4 = `${tz} *JUDUL* : ${nay2.result.title}\n`
            nay4 += `${tz} *SIZE* : ${nay2.result.size}\n`
            nay4 += `${tz} *KUALITAS* : ${nay2.result.quality}`
            sendImage(nay5, nay4)
            sendAudio(nay3)
            break
            case 'ytmp4':
            if (args.length < 1) return query("https://youtu.be/cfymgLi2CZc")
            nay2 = await fetchJson(`${restv01}/api/ytmp4/2?apikey=${apiv01}&url=${aq}`)
            reply(mess.wait)
            nay3 = await getBuffer(nay2.result.link)
            nay4 = `${tz} *JUDUL* : ${nay2.result.title}\n`
            nay4 += `${tz} *SIZE* : ${nay2.result.size}\n`
